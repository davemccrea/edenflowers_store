defmodule Edenflowers.Orders.Order.Validations.ValidateMinimumCartTotal do
  ...
  defp validate_minimum_cart_total(changeset, promotion_id) do
    with {:ok, promotion} <- Edenflowers.Pricing.Promotion.get_by_id(promotion_id, authorize?: false),
      ...
      if Decimal.compare(items_subtotal, minimum_required) in [:gt, :eq] do
        ...
        {:error,
         field: :promotion_id,
         message: ~t"Cart total must be at least #{Edenflowers.Format.currency(minimum_required)} to use this promotion"}
      end
    else
      {:error, _} ->
